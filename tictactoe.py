...
Type either "S" or "M" for either Single or Multi player modes
    ''')
    playerMode = str(input('Your Choice: '))
    while inputCheck:
        while not fullBoard(board):
            if not winCondition(board):
                if playerMode.lower() == 's':
                    inputCheck = False
                    if turnNumber % 2 == 0: # The turn number will always start from turn 0,if it is either 0 or an even number, then it means it is the player's turn
                        playerMarker = 'X' # e.g. turn 0, 2, 4...
                        printBoard()
            ...
            if winCondition(board): # This checks for whether any player has won while the board is filled up, otherwise, its a tie game
                if turnNumber % 2 == 0:
                    printBoard()
                    print("Games Over! Player 2 with the marker 'O' won!")
                    break
                ...
                print('Games Over! It is a tie game!')
                break
main()
